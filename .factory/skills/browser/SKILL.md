...
To use this skill, install the browser scripts at `~/.factory/skills/browser/`:

```bash
mkdir -p ~/.factory/skills/browser
# Copy start.js, nav.js, eval.js, screenshot.js, pick.js, package.json
npm install --prefix ~/.factory/skills/browser puppeteer-core
chmod +x ~/.factory/skills/browser/*.js
```

Then restart droid so it discovers the skill.
